...
) -> impl Responder 
{
	...
	let mut response = HttpResponse::Ok().body("Early return of API call, probably an error: There is no actual API response for the CLI wrapper currently");
	...
	if let Some(word) = words.next() {
		match word {
			...
			"quit" | "exit" => {
				println!("This does not do anything in the API");
			},
			_ => println!("Unknown command. See `\"help\" for available commands."),
		}
	}

	response = HttpResponse::Ok().body("No error was thrown when handling API request but there is no actual API response for the CLI wrapper currently");
	response
}
